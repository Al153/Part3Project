...
The category at each index should be a cartesian closed category.
...
\begin{itemize}
    \item An endo-functor indexed by the po-monad on effects: $\T{}{}: (\E, \dot\, \1,\subeffect)  \rightarrow \Cat(\C, \C)$
...
\begin{tikzcd}[ampersand replacement=\&]
    ...
    \& \&
     ...
     \\
    \T{\e_1\dot\e_2}{}
    \arrow [rrrr, "\deno{\e_1\dot\e_2\subeffect\e_1'\e_2'}"]
    \& \&
     \& \&
    \T{\e_1'\dot\e_2'}{}
\end{tikzcd}
...
\begin{equation}
    \treerule{Function Subtyping}{f = \deno{A' \subtype A} \s\s g = \deno{B \subtype B'}}{rhs = \deno{\fntype{A}{B} \subtype \fntype{A'}{B'}}: (B)^A \rightarrow (B')^{A'}}  
\end{equation}
...
\begin{align}
        rhs = & (g)^{A'}
        \after ({B})^f \\
        = & \cur{g\after \app} \after \cur{\app \after (\Id{{B}^{A'}} \times f)}\\
\end{align}
}

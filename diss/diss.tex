...
The $\okt$ predicate (Figure \ref{EffectEnvOk}) on effect environments asserts that the effect environment does not contain any duplicated effect variables. Using this, we can define the well-formedness relation on effects, $\wellformedeffect{\P}{\e}$ (Figure \ref{EffectWellformednes}). In short, this relation ensures that effects only reference variables that are in the effect environment. This is equivalent to saying that $\wellformedeffect{\P}{\e}$ means that $\e\in E_\P$.
...
\begin{framed}
    ...
    \]    
\end{framed}
    \caption{The well-formedness relation on typing environments}
...
Using this ground type relation, we can then construct a subtyping relation between pairs of types $A, B$ with respect to an effect environment $\P$ (Figure \ref{FullSubtypingDefinition}).
...
\begin{framed}
    ...
    \begin{proof}
        By induction on the derivation of $\D$ (Figure \ref{TypeRules}) using the denotations of terms (Figure \ref{TermDenotations}). Making use of the weakening of effect environments on term substitutions in the case of (\textit{\vgen})

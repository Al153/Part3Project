...
\begin{framed}
    \begin{definition}\label{MulDef}
        [$\Mul$ operator]
        $\Mul_n: \ciu\times \ciu \rightarrow \ciu = (f, g)\mapsto \ev\mapsto f(\ev)\dot g(\ev)$
    \end{definition}
    
    \begin{definition}[Naturality]\label{Naturality}
...
We are now equipped to define the denotations of structures in the language. Firstly, we define the denotation of effect-variable environments. In the base category, with the kind of effects indicated by $U$, the denotation of an effect-variable environment $\P$ is given by a finite product.

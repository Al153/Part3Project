...
\todo{Fix usage of rule name macros for \.subeffectree, \.trulecont}
...
\todo{Look at syntax highlighting this, esp differentiating type from effect}
\begin{framed}
    \begin{framed}
        \begin{verbatim}
def check[E: Effect](
...
When we specify a denotational semantics of a language in category theory, we look to find a mapping of types and term environments to objects in a specific categorical structure.  That is, there should exist objects $\deno{A}, \deno{\G}$ in the category $\C$ such that:
...
\todo{Explain syntax highlighting}
...
\begin{eqnarray}\label{PolymorphismTypeRules}
    ...
    \ntreeruleII{\vspec}{\gpetyperelation{v}{\all{\a}{A}}}{\wellformed{\P}{f}}{\gpetyperelation{\eapp{v}{f}}{A\ssub{\a}{f}}}
\end{eqnarray}
...
\todo{Be less vague abount this monoid + partial order}

Here effects $\e$ now range over the effect partially ordered monoid augmented with effect variables from an environment $\P = \nil, \a, \b ...$ (where $\nil$ represents the empty list), written as $(E_\P, \dotp, \subeffectp, \1)$. The ground effects of $E$ are now ranged over by $e$. We use $\e$ to range over effects including effect variables. The operator $\dotp$ is the extension of the original $\dot$ operator to symbolically act on expressions including effect variables. For example, if for some $\e$, $\forall \e'. \e'\dot\e = \e'$ then $\a\dotp\e = \a$ for some appropriate effect-variable environment $\P$.
...
\todo{This needs to be less vague.}
\todo{Adequately define the set, equational equality and partial order}

Here, $E_\P$ is the closure of $E\cup \left\{\a\mid\a\in\P\right\}$ under $\dotp$, which is defined as:

\begin{equation}
    \treeruleI{\e_3 = \e_1\dot\e_2}{\e_3 = \e_1\dotp\e_2}
\end{equation} 

For variable-free terms and is defined symbolically for variable-containing terms. Furthermore, we also define the subeffecting relation in terms of its variables and the ground relation.

\begin{equation}
    \e_1 \subeffectp \e_2 \Leftrightarrow \forall \si\downarrow. \e_1\sub{\si\downarrow} \subeffect \e_2\sub{\si\downarrow}
\end{equation}

Here $\si\downarrow$ denotes any ground-effect substitution of $\P$. That is any substitution of all effect variables in $\P$ to ground effects. Where it is obvious from the context, I use $\subeffect$ instead of $\subeffectp$.
...
The $\okt$ predicate on effect environments asserts that the effect environment does not contain any duplicated effect variables.
...
Using this, we can define the well-formedness relation on effects, $\wellformed{\P}{\e}$. In short, this relation ensures that effects only reference variables that are in the effect environment. This is equivalent to saying that $\wellformed{\P}{\e}$ means that $\e\in E_\P$.

\[
    \ntreeruleI{\eground}{\ok{\P}}{\wellformed{\P}{e}}
    \quad
    \ntreeruleI{\evar}{\ok{\P,\a}}{\wellformed{\P,\a}{\a}}
    \quad
    \condtreeruleI{\eweaken}{\wellformed{\P}{\a}}{\wellformed{\P,\b}{\a}}{\a\neq\b, \b\notin\P}
    \quad
    \ntreeruleII{\ecompose}{\wellformed{\P}{\e_1}}{\wellformed{\P}{\e_2}}{\wellformed{\P}{\e_1\dot\e_2}}
\]
...
\[
    ...
    \quad
    \ntreeruleII{\tfun}{\wellformed{\P}{A}}{\wellformed{\P}{B}}{\wellformed{\P}{\ab}}
    \quad
    \ntreeruleII{\teffect}{\wellformed{\P}{A}}{\wellformed{\P}{\e}}{\wellformed{\P}{\mea}}
    \quad
    \ntreeruleI{\tquant}{\wellformed{\P,\a}{A}}{\wellformed{\P}{\all{\a}{A}}}
\]
...
\[
    ...
    \quad
    \condtreeruleI{\envextend}{\oke{\P}{\G}\s\s \wellformed{\P}{A}}{\oke{\P}{\gax}}{x\notin\dom{\G}}
\]
...
\[
    \scalebox{0.85}{$
    \ntreeruleII{\vconst}{\oke{\P}{\G}}{\wellformed{\P}{A}}{\gpetyperelation{\const{A}}{A}} 
    \quad
...
\quad
\condtreeruleII{\vweaken}{\etyperelation{\P}{\G}{x}{A}}{\wellformed{\P}{B}}{\etyperelation{\P}{\gby}{x}{A}}{x \neq y, y\notin\dom{\G}}
\quad
...
\[
    ...
    \quad
    \ntreeruleII{\vspec}{\gpetyperelation{v}{\all{\a}A}}{\wellformed{\P}{\e}}{\gpetyperelation{\eapp{v}{\e}}{A\ssub{\a}{\e}}}
    $}
\]
\[
    \scalebox{0.85}{$
...
\begin{proof}
    We prove each of the implications separately, but an over-arching theme of this proof is that the leaf rules of any $\wellformed{\P}{\texttt{Consequence}}$ derivation tree require $\ok{\P}$ and that any modifications to the effect environment in the assumptions to the effect environment in the consequence of a rule preserve the $\okt$ relation. Hence, using induction and inversion, we can show that the effect environment is always well formed in any complete derivation tree.

    Firstly in the case of the well-formedness relation $\wellformed{\P}{\e}$, the only leaf rules are the (\textit{\eground}) and (\textit{\evar}) rules. By inversion, if $\wellformed{\P}{e}$ or $\wellformed{\P, \a}{\a}$ then $\ok{\P}$ or $\ok{\P, \a}$ respectively. Next, by inversion on the (\textit{\ecompose}) rule, if $\wellformed{\P}{\e_1\dot\e_2}$ then $\wellformed{\P}{\e_1}$, so by induction $\ok{\P}$. Finally, the weakening rule preserves the $\okt$ property of the effect environment, so $\wellformed{\P, \b}{\a} \implies \wellformed{\P}{\a} \implies \ok{\P} \implies \ok{(\P, \b)}$.

    Secondly, in the case of the well formedness relation on types, the only leaf rule is (\textit{\tground}). If $\wellformed{\P}{\g}$, then by inversion, $\ok{\P}$. Similarly to the (\textit{\ecompose}) rule for effects, the (\textit{\tfun}) rule's preconditions have the same effect-environment, so $\ok{\P}$ holds by inversion and induction. By inversion on the (\textit{\teffect}) rule, $\wellformed{\P}{\e}$ so $\ok{\P}$. Finally the (\textit{\tquant}) rule preserves $\okt$ in a similar way to the (\textit{\eweaken}) rule above.
...
\todo{More Signposting/Commentary}
...
In order to model the polymorphism of PEC, we need to now look at an indexed category. This consists of a base category, $\C$ in which we can interpret the possible effect environments in, and a mapping from objects in the base category to S-categories in the category of S-categories. This mapping is denoted from this point onwards as $\C(-)$ and the induced categories $\C(\deno{\P})$ are called \textit{fibres}. Furthermore, I shortly extend this mapping into a contravariant, \textit{S-preserving}, functor. The term ``S-preserving'' indicates that all functors derived from $\C$ within this category preserve the properties of S-categories, which are introduced in Definition \ref{SCategoryDefinition}. \todo{Lift this into a definition} A functor $F$ preserves the properties of S-categories if it preserves each of the features within an S-category. For example $F(A\times B) = (FA)\times (FB)$. For a full list of properties that an S-preserving functor should preserve, please see appendix \ref{AppendixReindexingFunctors}. To extend our mapping $\C(-)$ to a contravariant functor, it needs to map morphisms in the base category $\C$ to functors between fibres.  Thus, each morphism $\theta: \deno{\P'}\rightarrow\deno{\P}$ in $\C$ should induce an S-preserving, re-indexing functor $\theta\star: \C(\deno{\P})\rightarrow\C(\deno{\P'})$ between the fibres.
...
We are now equipped to define the denotations of structures in the language. Firstly, we define the denotation of effect environments. In the base category, with the kind of effects indicated by $U$, the denotation of an effect environment $\P$ is given by a finite product.

\[
\deno{\nil} = \1 \quad\quad \deno{\P, \a} = \deno{\P}\times U    
\]

Hence, the denotation of an  effect-variable environment $\P$ is given by a finite product on $U$ of width $n$ where $n$ is the length of $\P$. In order to de-clutter the notation used going forwards, inheriting from Crole (\cite{crole_1994}), I write $I$ as shorthand for $\deno{\P}$.  
...
\todo{The above gets broken up + awkward page break - check}
...
\todo{Figurify? + boxify?}
\todo{These are inconsistently spaced}
\[
...
By using the terminal objects and products present in each fibre, we can now derive denotations of type environments. $\deno{\oke{\P}{\G}}$ should be an object in the fibre $\C(I)$ induced by $\P$ .
\todo{Figurify? + boxify?}
\[
    \deno{\wellformedok{\P}{\nil}} = \1
    \quad\quad
...
\todo{These aren't very readable}
\todo{Figurify? + boxify?}
\[
...
As we go on, I define and state the denotations of specific substitutions and weakenings, upon both the effect environment and the term environments.
...
Substitutions are also an inductively defined relation between effect environments, with inductively defined denotations. Finite substitutions may be represented as a snoc-list of variable-effect pairs.
...
\[
    \ntreeruleI{Ni\esubnil}{\ok{\P'}}{\typerelation{\P'}{\nil}{\nil}}
    \quad\quad
    \condtreeruleII{\esubextend}{\typerelation{\P'}{\si}{\P}}{\wellformed{\P'}{\e}}{\typerelation{\P'}{\si, \a \setto\e}{(\P, \a)}}{\a\notin\P}
\]
...
The general construction for effect-variable substitution allows us to define, in particular, the identity substitution, $\typerelation{\P}{\Id{\P}}{\P}$ and the singleton substitution, $\typerelation{\P}{\ssub{\a}{\e}}{\P,\a}$.
...
These substitutions will form a useful shorthand later. 
\todo{lift this property into a definition block}
We also need to define the property $\a \# \si$. This indicates that $\a$ does not occur in the domain or any of the substitution expression of $\si$. Finally, it will be useful to examine how the substitutions generated by extending the effect environment, such as in the case of polymorphic types, relate to the original substitution.




\begin{theorem}[Effect Substitution on Effects]
     \todo{ordered list (i)}if $\wellformed{\P}{\e}$ and $\typerelation{\P'}{\si}{\P}$ then $\wellformed{\P'}{\si(\e)}$ \todo{(ii)} writing $\si$ for $\deno{\typerelation{\P'}{\si}{\P}}$,  $\deno{\wellformed{\P'}{\si(\e)}} = \deno{\wellformed{\P}{\e}}\after\si$. 
\end{theorem}

\todo{Boxify theorems}
\todo{More prose + signposting}
...
\begin{proof}
    ...
    \case{\eground}
    \todo{Add references to deno definition}
...
\begin{theorem}[Effect Weakening on Effects]\label{EffWeakeningEff}
   \todo{enumerate parts of theorem.} If $\wellformed{\P}{\e}$ and $\wrelw{\P'}{\P}$ then $\wellformed{\P'}{\w}$ and, writing $\w$ for $\deno{\wrelw{\P'}{\P}}$,  $\deno{\wellformed{\P'}{\e}} = \deno{\wellformed{\P}{\e}}\after\w$.

\end{theorem}

\todo{Move weakening versions of the proofs to appendices?}
\begin{proof}
...
Specifically, effect substitutions ($\si$) have the following actions on types, and type-environments:
...
\begin{theorem}[Effect Substitution on Types]
   If $\wellformed{\P}{A}$ and $\typerelation{\P'}{\si}{\P}$, then \todo{enumerate} $\wellformed{\P'}{A\ssi}$ and $\deno{\wellformed{\P'}{A\ssi}} = \si\star\deno{\wellformed{\P}{A}}$.
\end{theorem}

\begin{proof}
   By the fact that $\si\star$ is S-preserving and the Beck-Chevalley Condition.
   \todo{Justify why these cases}
    ...
    This case makes use of the Beck-Chevalley condition and the fact that $\deno{\typerelation{(\P', \a)}{(\si, \a\setto\a)}{(\P, \a)}} = \si\times\Id{U}$, which we can induct upon.
        ...
        $$\square$$
\end{proof}
\todo{"Similarly, the effect-substitution theorem on type environments is that"}
\begin{theorem}[Effect Substitution on Type Environments]
     \todo{enumerate} If $\wellformedok{\P}{\G}$, then $\wellformedok{\P'}{\G\ssi}$ and $\deno{\wellformedok{\P'}{\G\ssi}} = \si\star\deno{\wellformedok{\P}{\G}}$.
\end{theorem}

\begin{proof}
    By induction on the derivation of $\deno{\wellformedok{\P}{\G}}$ whilst making use of the S-preserving property of the re-indexing functor.
...
\begin{theorem}[Effect Weakening on Types]
    \todo{Enumerate}    
    If $\wrelw{\P'}{\P}$ and $\wellformed{\P}{A}$ then $\wellformed{\P'}{A}$ $\deno{\wellformed{\P'}{A}} = \w\star\deno{\wellformed{\P}{A}}$. Furthermore, $\wrelw{\P'}{\P}$ and $\wellformedok{\P}{\G}$ imply $\deno{\wellformedok{\P'}{\G}} = \w\star\deno{\wellformedok{\P}{\G}}$
\end{theorem}


\todo{Move proof to appendices}
\begin{proof}
   In a similar fashion \todo{to?}, inducting over the derivation of $\deno{\typerelation{\P}{A}{\type}}$, we make use of the Beck-Chevalley condition and the S-preserving property of $\si\star$.
    ...
    The proof for type environments follows the same steps as the effect-substitution proof.

    $$\square$$
\end{proof}

\todo{Add some big picture explaination}
\todo{More subsectioning?}
...
Similarly we can form the symmetrical weakening theorem. \todo{More big picture please}
...
The substitution theorem is expressed as so:
...
\begin{proof}
    This proof makes use of the previous effect-substitution theorems \todo{reference?}, and the adjunction of quantification and the re-indexing functor of projection, $\pstar$.
...
Going back to the original expression: \todo{needs a reference or something - too notey}
...
Now we are at a point to start considering weakenings and substitution of the term environments \todo{Why?}. \todo{Better linkage + big picture here} Type environment weakenings are inductively defined with respect to an effect environment.
...
The denotation of such a weakening is a morphism within the fibre category derived from the effect-variable \todo{Effect environment or effect-variable environment?} environment: $\deno{\pewrel{\w}{\G'}{\G}}: \G' \rightarrow \G \in \C(I)$. These denotations are defined as so:
...
Type-environment substitutions are also derived inductively with respect to an effect environment.
...
\todo{Put the action of substitutions here}
...
\todo{Make  road map more visible + big picture}
...
\todo{Move this proof to the appendices}
...
The term-substitution theorem is formulated similarly: 
...
\todo{Concluding remarks on this theorem}

\section{Uniqueness of Denotations}

\todo{Make this clear earlier}
Up until this point we have had to be careful about the implicit typing derivation \todo{Why} for every term denotation $\deno{\gpetyperelation{v}{A}}$. We are now equipped with the tools to prove that all derivations of the same typing relation instance $\gpetyperelation{v}{A}$ induce the same denotation. This allows us to no longer worry about the equality of denotations, which will be helpful in the soundness proof.
...
In particular a reduced typing derivation is one where subtyping rule only occurs in three places. Firstly, the subtyping rule must occur exactly once at the root of the tree in order to coerce the the typing relation to the correct type. Secondly, the subtyping rule must occur at the root of any of subtrees deriving the preconditions of the type-annotated (if) rule. This is to coerce the condition to a boolean type, and each of the branches to the type $A$ in the if-expression itself. Finally, the subtyping rule must occur at the root of the argument subtree of an (apply) rule. This allows us to coerce the argument to the correct type to match the parameter type in a lambda expression. These subtyping rule instances may be the identity subtyping rule, making use of the reflexive relation $A \subtypep A$. These rules have the effect of only introducing subtyping rule uses when it is necessary maintain syntactic correctness of the derivation. As an aside, if subtyping were a syntactic feature, such as being induced by explicit casts, then the rules for the typing relation would be entirely syntax directed, and hence we would not have the ambiguity that these theorems are required in order to solve. \todo{Such as the type annotation of lambda terms?}
...
\begin{equation}
    ...
    {\gpetyperelation{\lam{x}{A}{v}}{\fntype{A}{B'}}}
\end{equation}
    \caption{Two derivations of the same type relation. The second derivation is in reduced form.}
\end{figure}
...
\begin{figure}[h!]
    ...
    \begin{framed}
        ...
        \[\scalebox{0.8}{$
            \ntreeruleII{\eqeffbeta}{\wellformed{\P}{\e}}{\etyperelation{\P, \a}{\G}{v}{A}}{\gpeberelation{\eapp{(\elam{\a}{v}}{\e})}{v\ssub{\a}{\e}}{A\ssub{\a}{\e}}}
            \quad 
...
\begin{figure}[ht!]
   \begin{framed}
        \[\scalebox{0.8}{$
            ...
            \quad
            \ntreeruleII{\eqspec}{\gpeberelation{v_1}{v_2}{\all{\a}{A}}}{\wellformed{\P}{\e}}{\gpeberelation{\eapp{v_1}{\e}}{\eapp{v_2}{\e}}{A\ssub{\a}{\e}}}
        $}\]
...
\\
        ...
        $$\square$$
    \end{framed}
    \caption{Proof that the $\pstar$ and $\allEn$ functors form an adjunction, as required in \todo{Ref the required category structure chapter 3}}
...
\todo{needs more, comparison with System F}
\todo{introduce \.wellformedt, \.wellformede macros}

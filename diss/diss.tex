...
\todo{Type env vs term env?}
\todo{Use squares or don't use squares.}
\todo{Effect environment or effect-variable environment?}
...
Using this, we can define the well-formedness relation on effects, $\wellformedeffect{\P}{\e}$. In short, this relation ensures that effects only reference variables that are in the effect environment. This is equivalent to saying that $\wellformedeffect{\P}{\e}$ means that $\e\in E_\P$.
...
The well-formedness of effects can be used to a similar well-typed-relation on types, $\wellformedtype{\P}{A}$, which asserts that all effects in the type are well formed.
...
As stated earlier, the denotation of a well-formed effect is a morphism $\deno{\typerelation{\P}{\e}{\effect}}: I \rightarrow U$ in $\C$. The denotations for effects are shown in Figure \ref{EffectDenotations}
\begin{figure}[H]
    \centering
    \begin{framed}
        \[
...
Substitutions and weakenings are two distinct ways of manipulating effect or term environments. Weakening acts as a kind of subtyping of the environment. If we insert fresh variables into an environment, then any expression that was typeable under the previous environment should also be typeable under the the new environment. This change of environment should also have a predictable effect on the denotations of any expressions to which it is applied.
...
\todo{Reorder action and denotation, put in figures to improve space?}
...
These substitutions will form a useful shorthand later. Finally, it will be useful to examine how the substitutions generated by extending the effect environment, such as in the case of polymorphic types, relate to the original substitution.
...
\todo{Box + minipage-ify to put these next to each other}
\begin{align*}
    \g\sub{\si} &= \g \\
...
\begin{framed}
    ...
    \begin{proof}
       By induction on the derivation of $\deno{\wellformedtype{\P}{A}}$ and the fact that $\si\star$ is S-preserving. These specific cases show the dependencies on Theorem \ref{EffectSubstitutionOnEffects}, the Beck-Chevalley condition and the extension lemma (Lemma \ref{ExtensionLemmaOnEffectSubstitutions}).
...
\begin{framed}
    ...
    \begin{proof}
        This proof makes use of the previous effect-substitution theorems in section \ref{SubsAndWeakening}, and the adjunction of quantification and the re-indexing functor of projection, $\pstar$.
...
Having analysed the manipulation of the effect environment, we are now at a point to start considering weakenings and substitution of the term environments. Type environment weakenings are inductively defined with respect to an effect environment.
...
The denotation of such a weakening is a morphism within the fibre category derived from the effect-variable environment: $\deno{\pewrel{\w}{\G'}{\G}}: \G' \rightarrow \G \in \C(I)$. These denotations are defined as so:
...
Similarly to the definition of effect-environment substitutions (Section \ref{SectionEffectSubstitution}), type-environment substitutions are also snoc-lists derived inductively with respect to an effect environment.
...
\todo{Figure-ify and put next to denotations}
\begin{align*}
...
\begin{framed}
    ...
    \begin{proof}
        By induction on the derivation of $\D$. Making use of the weakening of effect environments on term weakenings in the case of (\textit{\vgen}). Some cases can be found in Appendix \ref{AppendixTermWeakeningTheorem}
    \end{proof}

...
\begin{itemize}
    ...
    \begin{itemize}
        \item Intro
        \item What is an effect?
        \item What is effect polymorphism?
        \item Introduction to Categorical Semantics     
    \end{itemize}
    \item Background \begin{itemize}
        \item Required Category Theory
        \item Languages and their requirements
        \item The Effect Calculus
        \item The Polymorphic Effect Calculus
    \end{itemize}

    \item The Semantics of PEC in an indexed category \begin{itemize}
        \item Semantics for EC
        \item Required Category Structure
        \item Road Map
        \item Denotations
        \item Substitution and Weakening theorems
        \item Unique Denotations
        \item Soundness
    \end{itemize}
...
\todo{What are effects, effect systems?}

\todo{STLC + Language requirements needs to be moved into the background}
...
\section{Strictly Indexed Category}
...
As suggested in \todo{Reference to intro}, since EC contains multiple effects, STLC terms, and \texttt{if} expressions, we should be able interpret its semantics in a cartesian closed category with an appropriate strong graded monad. With the addition of some extra category structure to handle subtyping, which I shall explain shortly, it is indeed possible to interpret EC. This section contains a fairly high-level treatment of the semantics. This is because the concepts introduced are a subset of those required for the semantics of PEC, which I shall explain in more detail later.
...
The leftwards and rightwards components of this bijection can be derived as follows. The leftwards component maps each morphism to a finite pairing of the morphism over each ground effect.

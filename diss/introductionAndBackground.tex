...
\abstract
    \todo{"improved upon by Lucasson": Katsumata}
...
\todo{
    "determines the effects that a program might have": the word
      determines makes it sound a bit like the effect system chooses the
      effects programs have.
}
\todo{- I think you might need to make the distinction between the actual
side-effects programs have at runtime, and the data tracked by an
effect system (which abstractly represent these side-effects).}
    
\todo{The symbol  is used for isomorphism between objects, so probably
      shouldn't be used for the equational theory as well.}
}
\section{What is an Effect System?}

Programs very rarely stand alone without interacting with their environment in some form. This interaction might be to receive input, such as from sensors or buttons, to write output to a file, or to halt with an error. In functional languages, manipulating mutable state can also be considered interacting with the environment. Language terms that produce these interactions are said to have \textit{effects} as they have some observable effect in the environment other than simply the value that executing the term produces. It is important to be able to reason about the effects that a program term may produce, in order to ensure the soundness of compiler transformations and formal verification. An effect system, analogously to a type system, is a formal system of rules which determines the effects that a program might have. Languages such as Haskell and the languages introduced later in this dissertation do not allow implicit effects. Instead they require the programmer to explicitly use structures called monads \needsRef{Moggi} to encode effect analysis in the type of program fragments. This has the useful result of combining the effect system with the type system of the language. For example, a program of type \texttt{Int} cannot interact with its environment to produce its result and hence always yields the same return value. However a program of type \texttt{IO Int} may perform IO operations before returning its result and hence may not always behave in exactly the same fashion.

\section{What is Effect Polymorphism?}


Effect polymorphism is when a single function in a language can operate on values of similar types but with different effects. It allows the same piece of code to be used in multiple contexts with different type signature. This manifests in a similar manner to type parameter polymorphism in system-F-based languages. Consider the following Scala-style pseudocode:
...
\chapter{Background}
\todo{Typo: "there exists morphism" should be "there exists a morphism".}

\todo{Typo: "between to functors" should be "between two functors". Also,
what is $\D$, D?}

\todo{- Preordered monoid: the multiplication should also be monotone.}
\todo{Definition of graded monad: shouldn't you give the definition for a
preordered monoid (rather than just a monoid?).}

\todo{ Defininition of tensorial strength: you should probably say *for a
graded monad* (it is also possible to define strength for functors
and other things).}

\todo{Adjunction: you don't actually say what an adjunction is!}
\todo{"weak functor": do you mean weak 2-functor?}
\todo{ CCCat: you should define this if you are going to mention it}
\todo{"calls a continuation": this might not be the best example, because
we normally wouldn't expect continuations to return}
\todo{"algebraic effect": avoid this phrase, since it means something
specific.}
\todo{Should 2.4 be in the Background chapter?}
\todo{The definition of the preordered monoid $E_\P$ isn't precise enough.}
\todo{Currently, $\gpetyperelation{v}{A}$ does not imply $\ok{\P}$. Perhaps it should?
(In general, it might be sensible to ensure that every judgment that
has a context $\P$ implies $\ok{\P}$.)}
...
\needsRef{Cite Mac Lane (categories for the working mathematician)?}
...
\begin{framed}
        ...
        \begin{minipage}{0.45\textwidth}
            ...
            \end{tikzcd}
            \caption{How the tensor strength natural transformation commutes with the join natural transformation \fixLayout{Fix overflow here}}
...
\todo{Think of an alternative for "S-Closed" ("S-Preserving?")}
\todo{"interpret its semantics in...": also need the coproduct 1+1. (Also,
this sentence is missing the word "to").}
\todo{Definition of S-category: you should say "an S-category is..." somewhere.}
\todo{A diagram might make the Beck-Chevalley condition easier to
understand.}
\todo{Section 3.5 inconsistently uses $\w$ and w for weakenings.} 
\todo{I think the (Project) and (Extend) rules should assume that $\a$ is not
already in the context.} 
\todo{ With inductively typed denotations: say what the type of $\deno{\w}$ is for an
arbitrary w before giving the definition.}
\todo{"This definition allows us to define": this doesn't use the definition
of substitution on effects.}
\todo{The statements of 3.5.1 and 3.5.2 have typos.} \todo{Why is the $\square$ at the end of each proof in the middle of the page?}
\todo{There are typos in the congruence rules for if and bind}
\todo{It might be a good idea to include the Effect-Eta case in the proof of
soundness.}
...
\begin{figure}
    \begin{framed}
        ...
        \fixLayout{Make these more readable/fix spacing}
        \[
...
\begin{figure}
    \begin{framed}
        \begin{tikzcd}
            &&   \deno{\typerelation{\P}{\all{\a}{A}}{\type}} \arrow{rrrr}{\counit{\deno{\typerelation{\P,\b}{A\ssub{\a}{\b}}{\type}}}}&&&&  \deno{\typerelation{\P}{A\ssub{\a}{\e}}{\type}} &&
            \\
            \G \arrow{rrrr}{g}&&&&
...
\todo{ Also, when you say
e.g. the subtyping rule can occur at the root, I think you mean there
must be *exactly* one use of the subtyping rule. Allowing zero would
also lead to non-uniqueness.
}
...
\begin{proof}
    ...
    \fixLayout{Make this and the other smaller}
    \begin{equation}
        \treerule{Type} {
            \treerule{Bind}{
                \edeltacrule{\G}{v_1}{\e_1}{A}{\e_1'}{A'}
...
\todo{"S-category properties": structure, not properties.}
\todo{"Morphisms $\E^n \rightarrow E$ in Eff are functions": monotone functions.}

    
\todo{"Since $\C_0$ is based on Set ... $[E^n, \C_0]$ is a CCC". It looks like $\C_0$ refers to the whole structure (i.e. a category (Set in this case),
graded monad, choice of products, etc), but when you state the functor
category, you use $\C_0$ as a category. I think you should write $[E^n, \Set]$
instead. (And I'm not sure you should name $\C_0$ at all.)} 
\todo{"we pick $\forall$ to be defined as a product": there is actually no choice
here: this functor has to be product. You might want to point this
out.}
\todo{ (4.63) has two typos.} 
\todo{ In (4.65), $(\e_n, e)$ should just be $\e_n$.}
\todo{(4.67) also has some typos.}
\todo{
    - Fig. 4.13 only has one of the triangle identities. Where is the other?} 
\todo{ - Merge Theorems 4.0.2 and 4.0.3.}
...
\fixLayout{This spacing will be fixed once I tweak around with my macros}
...
\begin{figure}
    \fixLayout{fix overflow here}
\fixLayout{Align frames}
...
\chapter{Conclusion}
\todo{ - "not addressed in presentations": this is too strong. Many people *do*
prove adequacy, but many don't.
}
\todo{- "optimizations to be made": give an example.}

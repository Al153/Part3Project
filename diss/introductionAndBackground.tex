...
\comment{
    ...
    - "improved upon by Lucasson": Katsumata
    ...
    - I think you might need to make the distinction between the actual
    ...
    - The symbol ≅ is used for isomorphism between objects, so probably
      shouldn't be used for the equational theory as well.
    
    # Chapter 2
    
    - Typo: "there exists morphism" should be "there exists a morphism".
    - Typo: "between to functors" should be "between two functors". Also,
      what is Δ? (D?)
    - Preordered monoid: the multiplication should also be monotone.
    - Definition of graded monad: shouldn't you give the definition for a
      preordered monoid (rather than just a monoid?).
    - Defininition of tensorial strength: you should probably say *for a
      graded monad* (it is also possible to define strength for functors
      and other things).
    - Adjunction: you don't actually say what an adjunction is!
    - "weak functor": do you mean weak 2-functor?
    - CCCat: you should define this if you are going to mention it
    - "calls a continuation": this might not be the best example, because
      we normally wouldn't expect continuations to return
    - "algebraic effect": avoid this phrase, since it means something
      specific.
    - Should 2.4 be in the Background chapter?
    - The definition of the preordered monoid `E_Φ` isn't precise enough.
    - Currently, `Φ | Γ ⊢ v : A` does not imply `Φ Ok`. Perhaps it should?
      (In general, it might be sensible to ensure that every judgment that
      has a context `Φ` implies `Φ Ok`.)
    
    # Chapter 3
    
    - "interpret its semantics in...": also need the coproduct 1+1. (Also,
      this sentence is missing the word "to").
    - Definition of S-category: you should say "an S-category is..."
      somewhere.
    - A diagram might make the Beck-Chevalley condition easier to
      understand.
    - Section 3.5 inconsistently uses ω and w for weakenings.
    - I think the (Project) and (Extend) rules should assume that α is not
      already in the context.
    - With inductively typed denotations: say what the type of ⟦w⟧ is for an
      arbitrary w before giving the definition.
    - "This definition allows us to define": this doesn't use the definition
      of substitution on effects.
    - The statements of 3.5.1 and 3.5.2 have typos.
    - Why is the □ at the end of each proof in the middle of the page?
    - There are typos in the congruence rules for if and bind
    - It might be a good idea to include the Effect-Eta case in the proof of
      soundness.
    
    # Chapter 4
    
    - "S-category properties": structure, not properties.
    - "Morphisms Eⁿ → E in Eff are functions": monotone functions.
    - "Since C₀ is based on Set ... [Eⁿ, C₀] is a CCC". It looks like C₀
      refers to the whole structure (i.e. a category (Set in this case),
      graded monad, choice of products, etc), but when you state the functor
      category, you use C₀ as a category. I think you should write [Eⁿ, Set]
      instead. (And I'm not sure you should name C₀ at all.)
    - "we pick ∀ to be defined as a product": there is actually no choice
      here: this functor has to be product. You might want to point this
      out.
    - (4.63) has two typos.
    - In (4.65), (εₙ, e) should just be εₙ.
    - (4.67) also has some typos.
    - Fig. 4.13 only has one of the triangle identities. Where is the other?
    - Merge Theorems 4.0.2 and 4.0.3.
    
    # Chapter 5
    
    - "not addressed in presentations": this is too strong. Many people *do*
      prove adequacy, but many don't.
    - "optimizations to be made": give an example.
}

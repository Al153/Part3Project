...
There's a bit of a problem (to me) with the introduction (1.2), because many things are done in quite some detail when what the reader needs is a bit of an intuitive view of what the problem
is and where the rest of the document is going.

For example I put a ring sound "cur" saying
"what's this", and only
found out what it is in chapter 2.  Even section 

1.1 "what is effect
polymorphism" assumes missing
text before it explaining what an effect is.

I agree with this. I think the introduction goes too technical too quickly. The part that talks the denotational semantics of STLC should probably go in the background. And there definitely needs to be more on what effects, effect systems, etc are.

The maths in Chapter 2 is fine, but it's can be a bit "brutal" (we do this, don't ask questions, or wonder why), e.g. section 2.8 on strength.  Read the first sentence in isolation! The critical idea that we need the operation of strength to be able to do various programming things is missing.
...
Section 3.3.6 isn't quite proper maths -- it's not that there exists, we're defining or assuming that subtyping is trivial between base types (or perhaps saying that a set of base types comes along with its own set of subtype relationships (int < real), but perhaps this is overkill here.
...
When you introduce $f*$ you should say "written", otherwise people wonder what it is (re-read the text assuming you're a newbie).  Ditto section 2.1.  The first sentence appears to be assuming things. So drop "recall that", and add "we explain these concepts below" just before 2.1.1.

You've taken hyphenation too much to heart; you don't need it in effect-ful (effectful) or sub-set. and not the final one in "polymorphic-effect-calculus" (actually you don't need either as it's a "polymorphic version of the effect calculus", but I agree that a "calculus with polymorphic effects" is also a good reading.).
